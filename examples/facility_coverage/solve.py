"""案例：2 km 园区步道照明覆盖设计（**几何 / 全称断言族**）

与案例 A（单 SKU 备货，随机优化族）完全不同的数学族：
  · 案例 A 的结论是"期望值最优"（随机、离散）；
  · 本案例的结论是"任意一点都被照亮"（确定性、全称量词、连续）。
但两者用的是**同一套验证引擎**，`troublesolver/checks.py` 一行没改——
这个案例的存在本身就是"泛化"的证据。
...
W = 4.0             # 步道宽度（m）
N_NOW = 7           # 现状灯杆数量
R_RATED = 166.667   # 出厂标称照明半径（m，工程取三位小数）
R_AGED = 166.6      # 实测照明半径（m，使用两年后光衰）
R_TARGET = 150.0    # 采购约束：能买到的灯杆半径上限（m）
...
def r_needed_from_layout(xs):
    """给定布点，覆盖全长所需**最小**半径 = 最大间距的一半。

    依据：相邻两灯杆之间的最不利点在中点，到最近灯杆的距离 = 间距/2；
...
def bounded_min_stations(r_target):
    """在"照明半径上限 r_target"的约束下，所需**最少**灯杆数（连续值）。
...
def main():
    ...
    m = {
        ...
        "coverage_now": {
            "r_needed_exact": worst_gap,
            "worst_point_x": worst_x,
            "shortfall_at_rated": worst_gap - R_RATED,     # 出厂标称下：负值=尚有微余量
            "shortfall_at_aged": worst_gap - R_AGED,       # 实测光衰后：正值=已经失守
    ...
    print("  长度 %.0f m，现状 %d 点等间距（间距 %.4f m）"
          % (L, N_NOW, d_now))
    print("  理论所需半径 %.4f m；细扫最坏点 x=%.4f m（距最近灯杆 %.4f m）"
          % (worst_gap, worst_x, worst_gap))
    print("  出厂标称半径 %.3f m → 余量 %.4f m（纸面上成立，实际是零余量）"
