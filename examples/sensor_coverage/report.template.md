...
- 案例：`examples/sensor_coverage`（几何 / 全称断言族）
- 数字来源：`out/metrics.json`（本报告不含任何手写数字）
- 复现命令：`python solve.py` ／ 复核命令：`mm audit examples/sensor_coverage`
- 台账：{{ledger.n}} 条结论，其中已证 {{ledger.verified}} 条、已证伪（留痕）{{ledger.refuted}} 条

## 一、结论（可直接执行）

**现状 7 点布点卡在临界点上：铭牌半径 {{m.params.r_rated}} m 时余量只有 {{m.coverage_now.margin_at_rated|.4f}} m（零点几毫米）；实测衰减半径 {{m.params.r_aged}} m 时已经失守。**
...
> 注意：**任何"刚好够"的等间距方案都是零余量**（覆盖 ⟺ 最大间距的一半 = 半径）。
...
| 项目 | 数值 | 说明 |
|---|---|---|
| 管廊长度 | {{m.params.L|.0f}} m | 宽 {{m.params.W|.0f}} m（本版只做中线几何覆盖） |
| 现状探测器数 | {{m.params.n_now}} 个 | 等间距 {{m.params.spacing_now|.4f}} m |
| 理论所需半径 | {{m.coverage_now.r_needed_exact|.4f}} m | 最坏点 x = {{m.coverage_now.worst_point_x|.2f}} m（相邻两点中点） |
| 铭牌标称半径 | {{m.params.r_rated}} m | 余量 {{m.coverage_now.margin_at_rated|.4f}} m ← **等于零余量** |
| 实测衰减半径 | {{m.params.r_aged}} m | 缺口 {{m.coverage_now.shortfall_at_aged|.4f}} m ← **已失守** |

> **口径警告（P6）**：铭牌半径与实测半径是两套口径，**不得混用**。
...
2. **实测衰减已经打破现状方案**：实测半径 {{m.params.r_aged}} m < 所需 {{m.coverage_now.r_needed_exact|.4f}} m，
   缺口 {{m.coverage_now.shortfall_at_aged|.4f}} m。这是"卡在临界点的设计"的典型后果。
3. **只做几何覆盖**：不含气体扩散、风向、探测器故障冗余（k-out-of-n）。含扩散时会显著改变覆盖定义。
...
- 查看：`mm ledger show examples/sensor_coverage/ledger.json`
- 对账：`mm ledger check examples/sensor_coverage/ledger.json`

## 六、复现方式

```bash
cd examples/sensor_coverage
python solve.py     # 重算 → out/metrics.json
mm solve .          # 卡点校验 → 自动跑检查器 → 台账 → 本报告
mm audit .          # 临时目录独立重跑 + 逐数字/逐检查对账
```
