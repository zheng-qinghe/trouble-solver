...
案例 B 的结论类型覆盖了：全称断言 / 最优性 / 稳健性 / 口径相关 ——
...
# ------------------------------------------------------------
def limit_samples():
    """① 只剩 2 个传感器：退化为"单段覆盖"，半径必须是全长的一半；
       ② 传感器足够多（11 个）时，半径必须严格小于 3 个传感器时的一半（单调性+可算）。"""
...
def flip_margin_n(n):
    """传感器数量的翻转判据：在采购半径上限约束下，n 个点是否够用。"""
